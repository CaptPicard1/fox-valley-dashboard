# ============================================================
# FOX VALLEY TACTICAL DASHBOARD v4.5 – Nov 2025
# Automated Intelligence Edition • One-File Stable Build
# ============================================================
...
# ---------- PAGE CONFIG ----------
st.set_page_config(
    page_title="Fox Valley Tactical Dashboard v4.5",
    layout="wide",
...
def get_latest_zacks_file(patterns):
    ...
    files = []
    for pat in patterns:
        for f in Path("data").glob(pat):
            m = date_pat.search(str(f))
            if m:
                files.append((m.group(1), f))
    return str(max(files)[1]) if files else None
...
def normalize_zacks(df):
    ...
    tickers = [c for c in df.columns if "ticker" in c.lower() or "symbol" in c.lower()]
    if tickers: df.rename(columns={tickers[0]:"Ticker"}, inplace=True)
...
with tabs[1]:
    ...
    else: st.info("No valid Zacks Growth 1 data detected.")
    ...
    if not g2.empty:
        ...
        st.dataframe(
            g2m.style.map(
                ...
                subset=["Zacks Rank"]),
            use_container_width=True)
    else: st.info("No valid Zacks Growth 2 data detected.")
    ...
    if not dd.empty:
        ...
        st.dataframe(
            ddm.style.map(
                ...
                subset=["Zacks Rank"]),
            use_container_width=True)
    else: st.info("No valid Zacks Defensive Dividend data detected.")
...
with tabs[4]:
    ...
    st.markdown("**🟢 New Zacks #1 Candidates (Not Held)**")
    st.dataframe(new,hide_index=True,use_container_width=True)
    st.markdown("**✔ Held Zacks Positions**")
    st.dataframe(held,hide_index=True,use_container_width=True)

# --- Daily Intelligence Snapshot ---
with tabs[5]:
    st.subheader("🧠 Daily Intelligence Snapshot – Auto Summary")
    ...
    if not combined.empty:
        ...
        st.metric("New Rank #1 Candidates",len(new_r1))
        st.metric("Held Rank #1 Positions",len(held_r1))
        st.metric("Portfolio Holdings",len(portfolio))

        if not new_r1.empty:
            st.markdown("**🟢 New Rank #1 Candidates**")
            st.dataframe(new_r1,use_container_width=True)
        if not held_r1.empty:
            st.markdown("**✔ Held Rank #1 Positions**")
            st.dataframe(held_r1,use_container_width=True)
    else:
        st.warning("⚠️ No Zacks data available for daily intelligence summary.")
...
with tabs[6]:
    ...
    st.write({
        "Rows – Growth 1": len(g1),
        "Rows – Growth 2": len(g2),
        "Rows – Defensive Dividend": len(dd),
        "Rows – Portfolio": len(portfolio)
    })
...
st.caption("Fox Valley Tactical Dashboard v4.5 build active – Automated Intelligence Edition")
